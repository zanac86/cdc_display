...
static uint8_t oled_buffer[OLED_BUFFERSIZE]; // Display buffer
static I2C_HandleTypeDef* hi2c; // Pointer I2C structure

static uint8_t pixel_oled_stress = 0;
...
uint8_t OLED_Init(I2C_HandleTypeDef* i2c_handleTypeDef)
{
    ...
    if (HAL_I2C_IsDeviceReady(hi2c, OLED_ADDRESS, 20, 200) != HAL_OK)
    {
        return 0;
    }
    ...
    OLED_SendCommand(OLED_HEIGHT - 1); // 0x3F);
...
void OLED_Invert(uint8_t inverted)
{
    if (inverted)
    {
        OLED_SendCommand(OLED_INVERT_DISPLAY);
    }
    else
    {
        OLED_SendCommand(OLED_NORMAL_DISPLAY);
...
void OLED_DrawBitmap(const uint8_t* progmem_bitmap, uint8_t height,
                     uint8_t width, uint8_t pos_x, uint8_t pos_y)
{
...
void OLED_DrawRectangleFill(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2,
                            uint8_t mode)
{
    uint8_t length = x2 - x1 + 1;
    uint8_t height = y2 - y1;
...
void OLED_DrawTriangle(int16_t x1, int16_t y1, int16_t x2, int16_t y2,
...
void OLED_DrawTriangleFill(int16_t x1, int16_t y1, int16_t x2, int16_t y2,
                           int16_t x3, int16_t y3)
{
...
void OLED_DrawCharPixelZoom(uint8_t ix, uint8_t iy, uint8_t color, uint8_t zoom)
{
    uint8_t x = ix;
    uint8_t y = iy;
    switch (zoom)
    {
        case 1:
            OLED_DrawPixelStatus(x, y, color);
            break;
        case 2:
            OLED_DrawPixelStatus(x + 0, y + 0, color);
            OLED_DrawPixelStatus(x + 1, y + 0, color);
            OLED_DrawPixelStatus(x + 0, y + 1, color);
            OLED_DrawPixelStatus(x + 1, y + 1, color);
            break;
        case 3:
        case 4:
            OLED_DrawPixelStatus(x + 0, y + 0, color);
            OLED_DrawPixelStatus(x + 1, y + 0, color);
            OLED_DrawPixelStatus(x + 2, y + 0, color);
            OLED_DrawPixelStatus(x + 0, y + 1, color);
            OLED_DrawPixelStatus(x + 1, y + 1, color);
            OLED_DrawPixelStatus(x + 2, y + 1, color);
            OLED_DrawPixelStatus(x + 0, y + 2, color);
            OLED_DrawPixelStatus(x + 1, y + 2, color);
            OLED_DrawPixelStatus(x + 2, y + 2, color);
            break;
        //  case 4:
        //      OLED_DrawPixelStatus(x + 0, y + 0, color);
        //      OLED_DrawPixelStatus(x + 1, y + 0, color);
        //      OLED_DrawPixelStatus(x + 2, y + 0, color);
        //      OLED_DrawPixelStatus(x + 3, y + 0, color);
        //      OLED_DrawPixelStatus(x + 0, y + 1, color);
        //      OLED_DrawPixelStatus(x + 1, y + 1, color);
        //      OLED_DrawPixelStatus(x + 2, y + 1, color);
        //      OLED_DrawPixelStatus(x + 3, y + 1, color);
        //      OLED_DrawPixelStatus(x + 0, y + 2, color);
        //      OLED_DrawPixelStatus(x + 1, y + 2, color);
        //      OLED_DrawPixelStatus(x + 2, y + 2, color);
        //      OLED_DrawPixelStatus(x + 3, y + 2, color);
        //      OLED_DrawPixelStatus(x + 0, y + 3, color);
        //      OLED_DrawPixelStatus(x + 1, y + 3, color);
        //      OLED_DrawPixelStatus(x + 2, y + 3, color);
        //      OLED_DrawPixelStatus(x + 3, y + 3, color);
        //      break;
        default:
            OLED_DrawPixelStatus(x, y, color);
            break;
...
void OLED_DrawCharPixelZoomStress(uint8_t ix, uint8_t iy, uint8_t color,
                                  uint8_t zoom, uint8_t stress)
{
    uint8_t x = ix;
    uint8_t y = iy;
    if (zoom < 4)
    {
        OLED_DrawCharPixelZoom(ix, iy, color, zoom);
        return;
    }
    if (stress == 0)
    {
...
void OLED_DrawCharZoom(char c, uint8_t ix, uint8_t iy, uint8_t mode,
                       uint8_t zoom)
{
    ...
    for (uint8_t x = 0; x < symbol[0]; x++)
    {
        ...
        {
            uint8_t tmp = symbol[1 + y / 8
                                 + x * (Font.numchars - 1) / Font.width]
                          & (0x01 << (y % 8));
            tmp = tmp ? m1 : m2;
            //          OLED_DrawCharPixelZoom(ix + x * zoom, iy + y * zoom, c, zoom);
            OLED_DrawCharPixelZoomStress(ix + x * zoom, iy + y * zoom, tmp,
                                         zoom, pixel_oled_stress);
        }
    }
}
...
void OLED_DrawChar(char c, uint8_t ix, uint8_t iy, uint8_t mode)
{
    ...
    for (uint8_t x = 0; x < symbol[0]; x++)
    {
        for (uint8_t y = 0; y < Font.height; y++)
        {
            if (symbol[1 + y / 8 + x * (Font.numchars - 1) / Font.width]
                & (0x01 << (y % 8)))
            {
...
void OLED_FastDrawChar(char c, uint8_t column, uint8_t row, uint8_t inverse)
{
    // only for 8 pixels height draw in one line

    if ((column >= (OLED_WIDTH / Font.width)) || (row >= (OLED_HEIGHT / Font.height)))
    {
        return;
    }
    ...
    uint16_t pos_x = row * OLED_WIDTH + column * Font.width;

    for (uint8_t x = 0; x < symbol[0]; x++, pos_x++)
    {
        uint8_t b          = symbol[1 + x] ^ inverse;
        oled_buffer[pos_x] = b;
    }
}

void OLED_FastDrawStr(const char* str, uint8_t column, uint8_t row, uint8_t mode)
{
    uint8_t stl = strlen(str);
    uint8_t pos = column;

    uint8_t max_column = OLED_WIDTH / Font.width - 1;
    uint8_t inverse    = (mode == 0) ? 0 : 0xff;
    ...
    if (pos == OLED_CENTER)
    {
        pos = (max_column - stl) / 2;
    }

    for (uint8_t cnt = 0; cnt < stl; cnt++, *str++, pos++)
    {
        OLED_FastDrawChar(*str, pos, row, inverse);
    }
}
...
void OLED_DrawStrZoom(const char* str, uint8_t x, uint8_t y, uint8_t mode,
                      uint8_t zoom)
{
    uint8_t stl = strlen(str);
    uint8_t pos = x;
    ...
    if (pos == OLED_RIGHT)
    {
        pos = OLED_WIDTH - OLED_GetWidthStr(str) * zoom - 1;
    }
    if (pos == OLED_CENTER)
    {
        pos = (OLED_WIDTH - OLED_GetWidthStr(str) * zoom - 1) / 2;
    }
    if (y == OLED_CENTER)
    {
        y = (OLED_HEIGHT - zoom * Font.height) / 2;
    }

    for (uint8_t cnt = 0; cnt < stl; cnt++, *str++)
    {
        OLED_DrawCharZoom(*str, pos, y, mode, zoom);
        pos += zoom * (fontbyte(Font.numchars * (*str - Font.offset) + 4));
    }
    pixel_oled_stress = (pixel_oled_stress + 1) % 2;
}
